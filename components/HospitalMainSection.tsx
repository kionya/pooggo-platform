...
export default function HospitalMainSection() {
  ...
  return (
    <section id="hospitals" className="py-20 px-4 bg-ivory">
            ...
            {hospitals.map((hospital) => {
              ...
              return (
                <div key={hospital.id} className={`bg-cream rounded-2xl overflow-hidden shadow-[var(--shadow-card)] border transition-all hover:shadow-lg ${isSelected ? 'border-gold-500 ring-2 ring-gold-500/20' : 'border-stone-200'}`}>
                  {/* 이미지 영역 */}
                  <Link href={`/hospitals/${hospital.id}`} className="block relative h-48 bg-stone-200 cursor-pointer group">
                    ...
                    <div className="absolute top-4 right-4 bg-cream/90 backdrop-blur px-2 py-1 rounded-lg flex items-center shadow-sm">
                  ...
                  <div className="p-6">
                    <Link href={`/hospitals/${hospital.id}`}>
                      <h3 className="text-xl font-bold text-navy-900 mb-1 hover:text-teal-600 transition">{resolveText(hospital.nameI18n, locale)}</h3>
                    </Link>
                    <div className="flex items-center text-sm text-stone-500 mb-3">
                      <MapPin className="w-4 h-4 mr-1" /> {hospital.location}
                    </div>
                    <p className="text-stone-600 text-sm line-clamp-2 mb-4 h-10">{resolveText(hospital.introI18n, locale)}</p>

                    <div className="flex flex-wrap gap-2 mb-6">
                      {/* 👇 안전장치 3: 태그 렌더링 시 에러 방지 */}
                      {tagsArray.slice(0, 3).map((tag: string, idx: number) => (
                        <span key={idx} className="bg-teal-600/10 text-teal-700 px-2 py-1 rounded-md text-xs font-medium">
                          #{tag}
                        </span>
                      ))}
                    </div>
      ...
      {compareList.length > 0 && (
        <div className="fixed bottom-0 left-0 w-full bg-cream border-t shadow-[0_-4px_20px_rgba(0,0,0,0.1)] p-4 z-50 animate-slide-up">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="bg-gold-500 text-navy-900 w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shadow-md">
                {compareList.length}
              </div>
